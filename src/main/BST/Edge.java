...
public class Edge {
    ...
    //checks if two edges share a wall, done by checking if either
    ...
    public boolean hasAdjacentWall(Edge candidateEdge) {
        if (hasSharedX(candidateEdge)) {
            return !areWallsSeparated(this.y1, this.y2, candidateEdge.y1, candidateEdge.y2);
        } else if (hasSharedY(candidateEdge)) {
            return !areWallsSeparated(this.x1, this.x2, candidateEdge.x1, candidateEdge.x2);
        }
    ...
    public boolean areWallsSeparated(int An1, int An2, int Bn1, int Bn2) {
